...
export default function ImmersionGallery() {
  ...
  return (
    <section id="gallery" className="bg-ink-950 relative z-10">
      ...
      {/* Lightbox — CSS transition only, mounted on demand */}
      {active !== null && createPortal(
        active !== null ? (
          <div
            className="fixed inset-0 z-[999] bg-ink-950/98 flex items-center justify-center animate-lightbox-fade"
            onClick={() => setActive(null)}
          >
            <div
              key={active}
              className="relative w-[90vw] h-[80vh] max-w-5xl animate-lightbox-scale"
